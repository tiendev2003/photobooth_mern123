...
export default function Step5() {
  ...
  const handleNext = async () => {
    if (paymentCode.length !== 4) {
      showDialog({
        header: "Thông báo",
        content: "Vui lòng nhập mã thanh toán 4 chữ số",
      });
      return;
    }
    setIsVerifying(true);
    try {
      ...
      const data = await res.json();
       if (data.isValid) {
        ...
        showDialog({
          header: "Thông báo",
          content: data.message || "Vui lòng kiểm tra lại mã thanh toán.",
        });
      }
    } catch (err) {
      ...
      showDialog({
        header: "Lỗi",
        content: "Đã xảy ra lỗi khi kiểm tra mã thanh toán. Vui lòng thử lại sau.",
      });
    } finally {
  ...
  return (
    <StoreBackground currentStore={currentStore}>
      <StoreHeader currentStore={currentStore} title="NHẬP MÃ THANH TOÁN" />
          ...
          {numbers.map((row, rowIndex) => (
            <div key={rowIndex} className="flex gap-4 md:gap-6 justify-center">
              {row.map((number) => (
                ...
                >
                  <span className="transition-transform text-5xl">
                    {number}
                  </span>
                </button>
              ))}
