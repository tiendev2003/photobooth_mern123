...
import {
  ...
  uploadImage,
  uploadVideo,
} from "@/lib/utils/universalUpload";
import {
  ChevronLeft,
  ChevronRight,
  ImageIcon,
...
export default function Step8() {
  ...
  const activeSkinFilter = useMemo(() => {
    return (
      skinFilters.find((filter) => filter.id === selectedFilter.id) ||
      skinFilters[0]
    );
  }, [selectedFilter]);
  ...
  const [filterCache, setFilterCache] = useState<Record<string, string[]>>({});

  // Thêm danh sách background mẫu
  // Lấy danh sách background từ API
  ...
  useEffect(() => {
    fetch("/api/backgrounds")
      .then((res) => res.json())
      .then((data) => {
        setBackgroundList(data.filter((bg: any) => bg.isActive));
        if (data.length > 0) setSelectedBackground(data[0]);
      });
  }, []);
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      const isLandscape = isCustomFrame
        ? false
        ...
        if (currentSessionCode) {
          try {
            ...
            if (updateResponse.ok) {
              ...
              console.error(
                "Failed to update media session:",
  ...
  const generateSmoothVideo = async (
    isLandscape: boolean
  ): Promise<string | void> => {
    try {
      const previewContent = printPreviewRef.current;
      ...
      const processedVideoPromise = new Promise<string>((resolve) => {
        mediaRecorder.onstop = () => {
          ...
          console.log(
            `Video file size: ${(finalBlob.size / (1024 * 1024)).toFixed(2)} MB`
          );
      ...
      const cellIndices = selectedFrame?.isCustom
        ...
        : Array.from(
            { length: selectedFrame!.columns * selectedFrame!.rows },
            (_, i) => i
          );
      ...
      if (videos.length > 0) {
        const selectedPhotoIndices = selectedIndices.filter(
          (idx) => idx !== undefined
        ) as number[];
            ...
            await new Promise<void>((resolve) => {
              ...
              videoElement.onloadedmetadata = () => {
                if (videoElement.videoWidth > 0) {
                  console.log(
                    `Video loaded with dimensions: ${videoElement.videoWidth}x${videoElement.videoHeight}`
                  );
                  resolve();
                }
              };
              ...
              videoElement.onerror = () => {
                console.error(
                  "Error loading video for high-quality processing"
                );
                resolve();
              };
      ...
      const videoStartPromises = Array.from(cellVideoMap.values()).map(
        async (video) => {
          try {
            video.loop = true;
            await video.play();
      ...
      console.log(
        "Video Generation - Frame type:",
        selectedFrame?.isCustom ? "Custom" : "Regular",
        "Grid container found:",
        !!gridContainer
      );
      console.log(
        "Video Preview canvas size:",
        previewCanvas.width,
        "x",
        previewCanvas.height
      );
      console.log(
        "Video Output canvas size:",
        outputCanvas.width,
        "x",
        outputCanvas.height
      );
      ...
      const renderFrame = () => {
        ...
        if (!anyPlaying && frameCount > targetFPS * 3) {
          // Ensure at least 3 seconds of recording
          mediaRecorder.stop();
          return;
        }
      ...
      setTimeout(() => {
        console.log(
          `Stopping video recording after ${recordingTimeout} seconds`
        );
      ...
      Array.from(cellVideoMap.values()).forEach((video) => {
        try {
          ...
          console.warn(
            "Error cleaning up video after smooth video generation:",
            error
          );
        }
      });
      ...
      activeVideoElementsRef.current.forEach((video) => {
        try {
          ...
          console.warn(
            "Error cleaning up video after smooth video error:",
            cleanupError
          );
        }
      });
  ...
  const generateGifFromVideo = async (
    ...
    try {
      ...
      console.log(
        "GIF Generation - Frame type:",
        selectedFrame?.isCustom ? "Custom" : "Regular",
        "Grid container found:",
        !!gridContainer
      );
      console.log(
        "GIF Preview canvas size:",
        previewCanvas.width,
        "x",
        previewCanvas.height
      );
      console.log(
        "GIF Output canvas size:",
        outputCanvas.width,
        "x",
        outputCanvas.height
      );
      ...
      if (gridContainer) {
        if (selectedFrame?.isCustom) {
          const cellElements = Array.from(gridContainer.children);
          console.log(
            "GIF Custom frame - Cell elements found:",
            cellElements.length
          );
          cellElements.forEach((cell, idx) => {
            if (cellVideoMap.has(idx)) {
              ...
              console.log(`GIF Custom cell ${idx}:`, {
                left: relativeLeft,
                top: relativeTop,
                width: cellRect.width,
                height: cellRect.height,
      ...
      for (let frameIndex = 0; frameIndex < totalFrames; frameIndex++) {
        ...
        if (isCustomFrame) {
          ...
          if (frameIndex === 0) {
            firstFrameData = outputCtx.getImageData(
              0,
              0,
              outputCanvas.width,
              outputCanvas.height
        ...
        if (frameIndex % Math.floor(totalFrames / 2) === 0) {
          console.log(
            `GIF progress: ${Math.round((frameIndex / totalFrames) * 100)}%`
          );
        }

        // Wait between frames - Giảm thời gian chờ để tăng tốc
        if (frameIndex % 2 === 0) {
          // Chỉ wait cho frames chẵn
          await new Promise((resolve) =>
            setTimeout(resolve, frameInterval / 2)
          );
        }
      }
        ...
        gif.on("finished", (blob: Blob) => {
          ...
          Array.from(cellVideoMap.values()).forEach((video) => {
            try {
              ...
              console.warn(
                "Error cleaning up video after GIF generation:",
                error
              );
            }
          });
      ...
      activeVideoElementsRef.current.forEach((video) => {
        try {
          ...
          console.warn(
            "Error cleaning up video after GIF error:",
            cleanupError
          );
        }
      });
      ...
      const canvas = await html2canvas(previewContent, {
        ...
        onclone: (clonedDoc) => {
          ...
          if (container && container.style) {
            ...
            if (
              selectedFrame?.columns === 1 &&
              selectedFrame?.rows === 1 &&
              !selectedFrame?.isCircle
            ) {
              container.style.paddingRight = "5%";
              container.style.paddingLeft = "5%";
            }
            container.style.aspectRatio = isCustomFrame
              ? "1/3"
          ...
          images.forEach((img) => {
            ...
            if (
              img.classList.contains("photo-booth-image") &&
              selectedFilter?.className
            ) {
              const filterString = convertFilterToCanvasString(
                selectedFilter.className
              );
          ...
          const previewContainer = clonedDoc.querySelector(
          ...
          if (previewContainer && selectedFilter?.className) {
            ...
            previewContainer.classList.add(
              ...selectedFilter.className.split(" ")
            );
          }

          const backgroundContainer = clonedDoc.querySelector(
          ...
          const overlayContainer = clonedDoc.querySelector(
          ...
          if (isCustomFrame) {
            ...
            if (gridElement) {
              gridElement.className =
                "relative z-10 grid grid-cols-1 gap-[20px]";
            }
          } else {
            const gridElement = clonedDoc.querySelector(".grid");
            if (gridElement && selectedFrame) {
              ...
              ).style.gridTemplateColumns = `repeat(${selectedFrame.columns}, 1fr)`;
            }
          }
        },
      });
  ...
  const handleFilterSelect = async (filter: (typeof skinFilters)[0]) => {
    ...
    // Nếu chọn "Bình thường" thì bỏ filter
    if (filter.id === "none") {
      setFilteredPhotos(null);
      return;
    }

    // Tạo key cache dựa trên filter và selectedIndices
    const cacheKey = `${filter.id}_${selectedIndices.join("_")}`;

    // Nếu đã có cache thì dùng luôn
    if (filterCache[cacheKey]) {
      setFilteredPhotos(filterCache[cacheKey]);
      return;
    }

    // Gọi API filter nếu chưa có cache
    try {
      const backendUrl =
        process.env.NEXT_PUBLIC_API_BACKEND ||
        (typeof window !== "undefined"
          ? (window as any).NEXT_PUBLIC_API_BACKEND
          : undefined) ||
        "http://127.0.0.1:5000";
      const formData = new FormData();
      // Chỉ gửi các ảnh đã chọn (theo selectedIndices)
      selectedIndices.forEach((photoIndex, idx) => {
        if (photoIndex !== undefined && photos[photoIndex]) {
          const photo = photos[photoIndex];
          // Nếu là base64 thì convert sang file, nếu là url thì fetch blob
          if (photo.image.startsWith("data:")) {
            // base64
            const arr = photo.image.split(",");
            const mime = arr[0].match(/:(.*?);/)?.[1] || "image/jpeg";
            const bstr = atob(arr[1]);
            let n = bstr.length;
            const u8arr = new Uint8Array(n);
            while (n--) u8arr[n] = bstr.charCodeAt(n);
            const file = new File([u8arr], `photo_${idx}.jpg`, { type: mime });
      ...
      formData.append("filter_name", filter.id);

      const res = await fetch(`${backendUrl}/api/apply-filter`, {
        method: "POST",
        body: formData,
      });
      if (!res.ok) throw new Error("Lỗi khi gọi API filter");
      const data = await res.json();
      // data.filtered_images: array of url (relative path)
      if (Array.isArray(data.filtered_images)) {
        // Đảm bảo mỗi url đều có tiền tố backendUrl
        const fullUrls = data.filtered_images.map((url: string) => {
        ...
        // Gán filteredPhotos vào đúng vị trí selectedIndices
        const filteredArr: string[] = [];
        selectedIndices.forEach((photoIndex, idx) => {
          if (photoIndex !== undefined) {
            filteredArr[photoIndex] = fullUrls[idx];
          }
        });
        setFilteredPhotos(filteredArr);
        // Lưu vào cache
        setFilterCache((prev) => ({ ...prev, [cacheKey]: filteredArr }));
      } else {
        throw new Error("API không trả về danh sách ảnh hợp lệ");
      }
    } catch (err) {
      console.error("Lỗi khi áp dụng filter:", err);
      setFilteredPhotos(null);
      alert(
        "Lỗi khi áp dụng filter: " + (err instanceof Error ? err.message : "")
      );
    }
  };
  ...
  const renderCell = (idx: number) => {
    ...
    if (photoIndex !== undefined) {
      if (filteredPhotos && filteredPhotos[photoIndex]) {
        imageUrl = filteredPhotos[photoIndex];
      } else {
        imageUrl = photos[photoIndex].image;
      }
    }
    ...
    <StoreBackground currentStore={currentStore}>
      ...
      <div className="grid grid-cols-2 gap-6 mx-32 z-30">
        ...
        <div className="lg:col-span-1">
          <div className="bg-zinc-200 rounded-2xl p-2 border border-indigo-500/30 flex flex-col gap-4">
                  ...
                  <div key={filter.id} className="px-2">
                    ...
                    >
                      ...
                      >
                        ...
                        {/* Bỏ tên filter */}
                        {/* <div className="p-2 text-center ...">{filter.name}</div> */}
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="px-2">
              ...
              <Slider {...slickSettings}>
                {backgroundList.map((bg) => (
                  <div key={bg.id} className="px-2 flex justify-center">
                    <div
                      onClick={() => setSelectedBackground(bg)}
                      className={`cursor-pointer rounded-full border-4 transition-all duration-200 ${
                        selectedBackground && selectedBackground.id === bg.id
                          ? "border-indigo-500"
                          : "border-transparent"
                      }`}
                      style={{
            ...
            <div className="px-2">
              ...
              <Slider ref={frameTemplateSliderRef} {...slickSettings}>
                {loading ? (
                  <div className="px-2">
                    <div className="bg-gradient-to-br from-indigo800/50 to-purple800/50 rounded-2xl flex items-center justify-center border border-indigo-500/30 aspect-square">
